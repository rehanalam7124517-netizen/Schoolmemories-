...
(function(){
  ...
  openSocialProfile=async function(name){
    try{
      ...
      const rg=b.querySelector('.profile-reel-grid');for(const r of reels){const cell=document.createElement('button');cell.dataset.r=r.id;cell.title='Open reel';const src=localMediaUrl(r,'reel');cell.innerHTML=src?`<video src="${src}" muted playsinline preload="metadata"></video><span>▶</span>`:'<div class="social-media-placeholder">🎬<small>ᴍᴇᴅɪᴀ ᴏɴ ᴏʀɪɢɪɴᴀʟ ᴅᴇᴠɪᴄᴇ</small></div>';cell.onclick=()=>{dlg.close();openReelsPage(r.id)};rg.appendChild(cell)}if(!rg.children.length)rg.innerHTML='<small>ɴᴏ ʀᴇᴇʟꜱ ʏᴇᴛ</small>';
